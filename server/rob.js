...
var visit = function(){
	//console.log(111);
	HTTP.get('http://mydemotest.meteor.com');
	interval = Math.random()*1000*180+1000*300;
...
var rob = function(start){
	...
	//visit();
	//Meteor.setTimeout(visit,600000);
	if(start == 1){
		data = getData();
		if(data.length == 0){
			rob(1);
	...
	if(typeof(param[hours]) != 'undefined' && param[hours]['success'] != 1){
		...
		try{
		robres = HTTP.post('http://luck.dxt.cn/v2/luck/rob',{params:{luck_id:param[hours]['luck_id'],sign:param[hours]['sign'],user_id:param[hours]['user_id']},headers:{'Content-Type':'application/x-www-form-urlencoded'}});
		robdata = robres.data;
